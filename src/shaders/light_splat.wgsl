...
@group(0) @binding(0) var<storage, read_write> light_gpu_buffer: array<vec4<f32>>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    var accumulated_light = vec4<f32>(0.0);
    ...
    for (var i = 0u; i < num_instances; i = i + 1u) {
        ...
        if (dist < radius) {
            ...
            accumulated_light += inst.light_fields * weight;
            total_weight += weight;
        }
    }

    if (total_weight > 0.0) {
        light_gpu_buffer[idx] = accumulated_light / total_weight;
    } else {
        light_gpu_buffer[idx] = vec4<f32>(0.0);
    }
}
